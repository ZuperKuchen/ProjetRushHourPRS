...
int main(int argc, char *argv[]){
  ...
  piece* tab2_pieces=(piece*)malloc(sizeof(piece));;
  tab2_pieces[0]=new_piece(0,3,2,1,true,true);
  tab2_pieces[1]=new_piece(1,2,3,1,true,true);
  tab2_pieces[2]=new_piece(4,0,1,3,true,true);
  game testg2 = new_game (4,5,3,tab2_pieces);
  result = result && test_game_nb_piece(testg2, 3);
  result = result && test_game_piece(testg2, 2, 4, 0);
  result = result && !(is_out(tab_pieces[0],testg));
  //result = result && test_play_move(testg,di);
